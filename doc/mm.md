...
```
After User confirms a quotation, User is required to deposit token X to RFQ contract on chain A, by calling [srcDeposit](https://github.com/celer-network/sgn-v2-contracts/blob/opt/contracts/message/apps/RFQ.sol#L80).
...
a hacked or malicious server. If everything goes well, then MM can call [dstTransfer](https://github.com/celer-network/sgn-v2-contracts/blob/opt/contracts/message/apps/RFQ.sol#L136) to transfer token Y to User on chain B.
During `dstTransfer`, 
...
```
When MM got the proof of order fulfillment, MM can call [srcRelease](https://github.com/celer-network/sgn-v2-contracts/blob/opt/contracts/message/apps/RFQ.sol#L188) to release token X on chain A. During `srcRelease`,
